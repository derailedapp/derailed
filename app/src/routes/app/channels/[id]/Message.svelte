...
let isAuthor = $derived(author?.id == $currentActor?.id);
...
<ContextMenu.Root bind:open={ctxOpen}>
	<ContextMenu.Trigger>
		<li id={message.id} class="flex flex-row w-full group pl-6 p-1 py-0.5" class:gap-3={!cascade} class:gap-1={cascade} class:flex-row-reverse={author?.id == $currentActor?.id} class:my-2={!cascade && !nextMessageWillCascade} class:pl-7={cascade && !isAuthor} class:pr-7={cascade && isAuthor} class:items-center={cascade}>
			{#if !nextMessageWillCascade && !isAuthor}
			...
			<div class="flex flex-col">
				<div class="flex gap-2 items-center" class:hidden={cascade}>
					<UserPopUp user={author!}>
						<div class="hover:underline text-white" class:hidden={author?.id == $currentActor?.id}>
							{author?.display_name || author?.username || "Deleted User"}
						</div>
					</UserPopUp>
					<div class="text-weep-gray tracking-tighter text-xs select-none">
						{getDate(message.created_at)}
					</div>
				</div>
				<div class="text-white bg-blurple font-light text-[16px] Markdown w-auto m-auto">
	...
	<ContextMenu.Portal>
		<ContextMenu.Content class="flex flex-col items-center text-white/85 tracking-tight text-sm gap-1 w-[200px] p-1.5 rounded-md bg-secondary-bg select-none border border-weep-gray">
